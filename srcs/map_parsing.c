...
/*   Updated: 2022/02/21 13:27:16 by bterral          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	read_map(int fd, t_mlx *mlx)
{
	...
	if (is_not_full_walls(current_line))
		return (free_and_error(&current_line, &mlx->map, 2));
	while (current_line)
	{
		mlx->map = add_line_to_map(current_line, &*mlx);
		if (!mlx->map)
			free_and_error(&current_line, &mlx->map, 11);
		if (ft_line_len(current_line) != mlx->x)
			return (free_and_error(&current_line, &mlx->map, 3));
		if (is_not_wall_surrounded(current_line))
			return (free_and_error(&current_line, &mlx->map, 4));
		free(current_line);
		current_line = get_next_line(fd);
		mlx->y++;
	}
	if (is_not_full_walls_last_line(mlx->map))
		return (free_and_error(&current_line, &mlx->map, 5));
	close(fd);
